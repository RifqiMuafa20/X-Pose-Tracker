...
class ResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(recordType == 0){
            recordTitle = generateDefaultRecordName()
        } else if(recordType == 2){
            recordTitle = intent.getStringExtra("record_name") ?: ""
        }
        ...
        if(recordType == 1){
            binding.recordName.visibility = View.GONE
            binding.editButton.visibility = View.GONE
        } else {
            binding.recordName.text = recordTitle

            binding.editButton.setOnClickListener {
                val dialog = EditTextDialog(binding.recordName.text.toString()) { newText ->
                    binding.recordName.text = newText
                    recordTitle = newText
                    ...
                    if (recordType == 2) {
                        ...
                        viewModel.updateUserRecord(userRecord)
                    }
                }
    ...
    private fun generateDefaultRecordName(): String {
        val timeOfDay = getString(DateHelper.getTimeOfDay())
