...
class HistoryAdapter : RecyclerView.Adapter<HistoryAdapter.ActivityViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ActivityViewHolder, position: Int) {
        ...
        item.recordPhotos?.let {
            holder.image.setImageBitmap(it.toBitmap())
        } ?: holder.image.setImageResource(R.drawable.gym_background)

        holder.itemView.setOnClickListener {
            val intent = Intent(it.context, ResultActivity::class.java)
            intent.putExtra("record_id", item.idRecord)
            intent.putExtra("record_name", item.recordName)
            ...
            intent.putExtra("user_id", item.idUser)
            item.recordPhotos?.let {
                intent.putExtra("record_photo_bytes", it)
            } ?: intent.putExtra("record_photo_res", R.drawable.gym_background)
